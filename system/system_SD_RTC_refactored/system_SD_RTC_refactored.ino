...
void add_smoothing_data() {
  ...
  total_soil_mositure = total_soil_mositure - readings_temp[readIndex];
...
void log_into_SDcard(){
  ...
  if(myFile){
    if(!Serial1.available()){
      myFile.print(create_sensors_actuators_log_string(CONN_ERR));
      log_esp_communication_failed_into_SDcard();
    }
    else{
      myFile.print(create_sensors_actuators_log_string(NO_ERR));
...
String create_sensors_actuators_log_string(int statusFlag){
  return String(myRTC.year) + "/" + String(myRTC.month) + "/" + String(myRTC.dayofmonth)
            ...
            + "," + String(loop_pH) + "," + String(average_soil_mositure) + "," 
